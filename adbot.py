...
DEFAULT_CONFIG = {
    ...
    "announcement_message": " خوش امدین به هتل پسرونه امیدوارم لحظات خوشی را سپری کنید😀برای ادمین شدن به مالک پیام دهید😀برای مشکل های کوچک به ادمین ها پیام دهید برای مشکل های بزرگ به مالک😀 فوش ممنوع❎️ "
}
...
class AdvancedBot(BaseBot):
    ...
    async def cmd_help(self, user: User, parts: list):
        help_text = (
            "<#F5F3FF>دستورات ربات:\n"
            "<#B8FFD8>1-6 - اجرای رقص\n"
            "<#8FF7FF>stop - توقف رقص\n"
            "<#FFD700>!help - نمایش راهنما\n"
            "<#F5F3FF>!spam تعداد پیام - ارسال پیام اسپم\n"
            "<#8FF7FF>!tele @username [vip|vip1|dj|مکان_سفارشی] - تلپورت کاربر\n"
            "<#B8FFD8>!tele to @username - تلپورت به کاربر\n"
            "<#F5F3FF>!tele me @username - تلپورت کاربر به ادمین\n"
            "<#8FF7FF>!tele me all - تلپورت همه به ادمین\n"
            "<#F5F3FF>!heart تعداد @username - ارسال قلب بنفش\n"
            "<#FFD700>!heart all - قلب بنفش به همه\n"
            "<#8FF7FF>>!clap تعداد @username - ارسال clap\n"
            "<#F5F3FF>!clap all - clap به همه\n"
            "<#8FF7FF>!wink تعداد @username - ارسال wink\n"
            "<#F5F3FF>!wink all - wink به همه\n"
            "<#8FF7FF>!wave تعداد @username - ارسال wave\n"
            "<#FFD700>!wave all - wave به همه\n"
            "<#8FF7FF>!thumbs تعداد @username - ارسال thumbs-up\n"
            "<#8FF7FF>!thumbs all - thumbs-up به همه\n"
            "<#FFD700>!wallet - نمایش موجودی ربات\n"
            "<#8FF7FF>!set - تلپورت ربات به ادمین\n"
            "<#FFD700>!item set @username - تغییر ظاهر ربات به ایتم‌های کاربر\n"
            "<#8FF7FF>!tip <تعداد> all - تیپ به همه\n"
            "<#FFD700>!vip - تلپورت به VIP\n"
            "<#F5F3FF>!vip1 - تلپورت به VIP1\n"
            "<#F5F3FF>!dj - تلپورت به DJ\n"
            "<#8FF7FF>!down - تلپورت به پایین\n"
            "<#F5F3FF>!ban @username - بن کردن کاربر\n"
            "<#8FF7FF>!unban @username - آنبن کردن کاربر\n"
            "<#8FF7FF>!dancechain - اجرای زنجیره رقص\n"
            "<#8FF7FF>!addtele نام_مکان - ذخیره مکان جدید\n"
            "<#8FF7FF>!deltele نام_مکان - حذف مکان تلپورت\n"
            "<#8FF7FF>!welcome پیام - تنظیم پیام خوش‌آمدگویی\n"
            "<#8FF7FF>!addadmin @username - افزودن ادمین (فقط Host)\n"
            "<#8FF7FF>!removeadmin @username - حذف ادمین (فقط Host)\n"
            "<#FFD700>!emotebot نام/شماره_دنس - تغییر دنس مداوم ربات (فقط ادمین)\n"
            "<#FFD700>!loopchat پیام - تنظیم پیام تکرارشونده/اسپم ربات (فقط ادمین)\n"
            "<#8FF7FF>!listadd - نمایش لیست ادمین‌ها\n"
            "<#8FF7FF>!freeze @username - فریز کردن کاربر\n"
            "<#F5F3FF>!unfreeze @username - آزاد کردن کاربر از فریز\n"
            "<#B8FFD8>!party @username عدد - اجرای رقص اجباری برای کاربر\n"
            "<#F5F3FF>!party all عدد - اجرای رقص برای همه\n"
            "<#B8FFD8>!partys @username - توقف رقص اجباری کاربر\n\n"
            "<#FFD700>📩 برای اطلاعات بیشتر به @800.98 پیام بدید!"
        )
